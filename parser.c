...
int main() {
    // Create a simple schema for a single unsigned integer
    Schema* schema = sint_array_schema();

    int sint_arr[] = {1, -2, 3};
    ParsedData* test_data = array_sint_data(sint_arr, 3);

    // Pack the data
    char* packed_data = NULL;
    size_t packed_size = 0;
    int pack_result = pack_with_schema(test_data, schema, &packed_data, &packed_size);
    if (pack_result != 0) {
        printf("Packing failed with error code: %d\n", pack_result);
        free_parsed_data(test_data);
        free_schema(schema);
    ...
    const char* unpack_ptr = packed_data;
    size_t remaining = packed_size;
    ParsedData* unpacked_data = unpack_with_schema(&unpack_ptr, &remaining, schema);
    if (!unpacked_data) {
        printf("Unpacking failed\n");
        free(packed_data);
        free_parsed_data(test_data);
